(function() {
  ...
  AWS.config.update({
    ...
    credentials: {accessKeyId: appInfo.AccessKeyForUnauthenticatedUser, secretAccessKey: appInfo.SecretKeyForUnauthenticatedUser},
    logger: console
  });
  ...
  var adminCredentials = new AWS.WebIdentityCredentials({
    RoleArn: appInfo.AdminRoleARN,
    ProviderId: appInfo.providerId
  });

  // Setup some service objects
  var dbReader = new AWS.DynamoDB({params: {TableName: appInfo.PostTableName}});
  var dbWriter = new AWS.DynamoDB({
    params: {TableName: appInfo.PostTableName},
    credentials: adminCredentials
  });
  var s3Bucket = new AWS.S3({
    paramValidation: false,
    computeChecksums: false,
    params: {Bucket: appInfo.BucketName},
    region: appInfo.Region,
    credentials: adminCredentials
  });
    ...
    if (file) {
      ...
      s3Bucket.putObject(params, function (err, data) {
        if (err) {
          ...
          var url = '//' + appInfo.BucketName + '.s3.amazonaws.com/' + params.Key;
          articleBody.value += '\n\n![](' + url + ')\n\n';
        }
      });
    }
  ...
  window.fbAsyncInit = function() {
    FB.init({appId: appInfo.FacebookAppID});
